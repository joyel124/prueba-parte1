...
def normalize_estado(s: str) -> str:
    ...
    if s in ("EN PROCESO","ENPROCESO"): return "EN_PROCESO"
    if s in ("PRODUCTIVO","TERMINADO"): return "PRODUCTIVO"
    if s in ("DOC ENVIADO","DOCENVIADO","DOC.ENVIADO","DOCUMENTACION ENVIADA","DOC"): return "DOC_ENVIADO"
    return "PENDIENTE"
...
<div class="ui-panel" id="uiPanel">
  ...
  <div class="ui-filters">
    <div>
      ...
      <select id="uiEstado" class="ui-select">
        ...
        <option value="DOC_ENVIADO">Doc. Enviado</option>
      </select>
    </div>
    <div>
  ...
  <div class="ui-legend">
    ...
    <div class="ui-leg-item">
      <div class="ui-leg-sym bx" style="background:#2563EB;"></div>
      <span class="ui-leg-label">Doc. Enviado</span>
    </div>
  </div>
...
(function(){
  ...
  function showTooltip(nodeId, mouseX, mouseY){
    ...
    var estadoDisp=(d.estado||"");
    var estadoMap={"PENDIENTE":"Pendiente","EN_PROCESO":"En Proceso","PRODUCTIVO":"Productivo","DOC_ENVIADO":"Doc. Enviado"};
    estadoDisp=estadoMap[estadoDisp]||(estadoDisp.replace("_"," ").replace(/\w\S*/g,function(t){return t.charAt(0).toUpperCase()+t.substr(1).toLowerCase();}));
  ...
  // Interceptar wheel ANTES de que vis lo procese.
  // Si ya estamos en el límite y el scroll va en esa dirección → cancelar el evento
  // para que vis nunca reciba el delta y no desplace el viewport.
  var canvasEl = container.querySelector("canvas");
  if(canvasEl){
    canvasEl.addEventListener("wheel", function(e){
      var s = network.getScale();
      // deltaY > 0 = alejar (zoom out), deltaY < 0 = acercar (zoom in)
      var zoomingOut = e.deltaY > 0;
      var zoomingIn  = e.deltaY < 0;
      if((zoomingOut && s <= ZOOM_MIN) || (zoomingIn && s >= ZOOM_MAX)){
        e.preventDefault();
        e.stopPropagation();
      }
  ...
  // Scroll/pinch en canvas → slider (ya no necesita clamp reactivo, el wheel lo previene)
  network.on("zoom", function(){
    var s = network.getScale();
    // Clamp de seguridad por si llega via pinch o teclado
    if(s < ZOOM_MIN){ network.moveTo({scale: ZOOM_MIN}); return; }
    if(s > ZOOM_MAX){ network.moveTo({scale: ZOOM_MAX}); return; }
    syncSlider();
  });
  ...
  // Función centralizada de fit — actualiza ZOOM_MIN dinámicamente
  function doFit(animated){
    var opts = {animation: animated ? {duration:350, easingFunction:"easeInOutQuad"} : false};
    network.fit(opts);
    setTimeout(function(){
      ZOOM_MIN = network.getScale();
      // Actualizar slider min para reflejar nuevo límite
      zoomSlider.min = Math.round(ZOOM_MIN * 100);
      syncSlider();
    }, animated ? 400 : 50);
  }

  btnFit.addEventListener("click", function(){ doFit(true); });
